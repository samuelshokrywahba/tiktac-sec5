...
def recieveThread(session) :
    while True :
        decodedmessage = session.recv(2048).decode('utf-8')
        if decodedmessage=='a':
           btn1["text"]='X'
           check()  
        if decodedmessage=='b':
...
def clientThread(session) :
    ## creating thread and start it 
    recvThread = threading.Thread(target=recieveThread,args=(session,))
    recvThread.start()
## creating socket and listen for clients'
...
window.title("tic tac toe FOR SERVER")
window.geometry("400x300")
#create labels for the two players and set text and font type and size  
player1=Label(window,text="CLIENT:X    ",font=('Courier','15'))
player1.grid(row=0,column=0)
player2=Label(window,text="SERVER:O    ",font=('Courier','15'))
...
while True :
    session, address = socket.accept()
    print('Connection from ',address[0])
    start_new_thread(clientThread,(session,))
